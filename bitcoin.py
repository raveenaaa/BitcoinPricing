...
def computeDelta(wt, X, Xi):
    ...
    Parameters
    ----------
    e = exp(wt*summ((az-mean(a)(bz-mean(b)))/M*std(a)std(b)))
    E[y|x] = summ(yi*e)/summ*(e)    
    wt : int
    ...
    """
	az = X[0:len(X.index)-1]
	mean_a = np.mean(az)
	std_a = np.std(az)
	def compute_similarity(Xi):
		bz = Xi[0:len(Xi.index)-1]
		mean_b = np.mean(bz)
		std_b = np.std(bz)
		similarity = math.exp(wt*((bz - mean_b)*(az - mean_a)).sum()/(len(az.index)*std_a*std_b)) 
		return similarity    

	similarity = Xi.apply(compute_similarity, axis = 1)
	prediction = sum(Xi['Yi']*similarity)/similarity.sum()
	return prediction
...
# YOUR CODE HERE
formula = smf.ols(formula = 'deltaP ~ deltaP90 + deltaP180 + deltaP360', data = trainData)
model = formula.fit()
...
# YOUR CODE HERE
MSE = sm.mean_squared_error(compareDF.loc[:, ['Actual']], compareDF.loc[:, ['Predicted']])
